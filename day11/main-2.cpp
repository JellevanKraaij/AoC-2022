...
struct Monkey {
    ...
    bool executeItem(int &monkey, uint64_t &item, uint64_t normalizeVar);
    void executeOperation(uint64_t normalizeVar);
...
void Monkey::executeOperation(uint64_t normalizeVar) {
    ...
    items.front() %= normalizeVar;
}

bool Monkey::executeItem(int &monkey, uint64_t &item, uint64_t normalizeVar) {
    if (items.empty())
        return (false);
    executeOperation(normalizeVar);
    item = items.front();
    items.pop_front();
...
int main(int argc, char **argv) {
    ...
    uint64_t normalizeVal = 1;
    for (const pair<Monkey, int> &monkeyPair : monkeys) {
        normalizeVal *= monkeyPair.first.divisionValue;
    }

    for (size_t rounds = 0; rounds < 10000; rounds++) {
        for (size_t i = 0; i < monkeys.size(); i++) {
            int destMonkey;
            uint64_t item;
            while (monkeys.at(i).first.executeItem(destMonkey, item, normalizeVal)) {
                monkeys.at(i).second++;
